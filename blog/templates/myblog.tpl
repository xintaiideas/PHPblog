	...
	<div id="section" class="box">
		...
		<ul class="articles box">
			...
			<div class="pagination box">
				<p class="f-right">
					{{if $pageInfo.page > 1}}
						<a href="http://localhost/blog/?action=myblog&page={{$pageInfo.page-1}}">上一页 &raquo;</a>
					{{/if}}
					{{foreach from=$pageInfo.page_arr item=item}}
						<a href="http://localhost/blog/?action=myblog&page={{$item}}" {{if $item==$pageInfo.page}}class="current"{{/if}}>{{$item}}</a>
					{{/foreach}}
					{{if $pageInfo.page < $pageInfo.page_max}}
					<a href="http://localhost/blog/?action=myblog&page={{$pageInfo.page+1}}">下一页 &raquo;</a>
					{{/if}}
				</p>
